        ...
        <div class="row">
          <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
              ...
              <li><i class="fa fa-table"></i><a href="pendaftar_reguler.php">Pendaftar Reguler</a></li>
            </ol>
          </div>
        </div>
        <div class="row">
            ...
            <section class="panel">
              ...
              <table class="table table-striped table-advance table-hover">
                <tbody>
                  ...
                  <?php
                  error_reporting(0);
                  $sql = mysqli_query($con,"SELECT * FROM peserta_reguler WHERE status='0' ORDER BY kd_pr ASC");
                  while ($row = mysqli_fetch_array($sql)) {
                    ...
                    <tr>
                      ...
                      <td>$row[alamat]</td>
                      <td>$row[paket]</td>
                      <td>$row[mapel_tambah]</td>
                      <td>$row[jadwal]</td>
                      <td><span class=\"badge badge-warning\">Belum Konfirmasi</span></td>
                      <td>
                      <button type=\"button\" class=\"btn btn-success\" onclick='peserta(\"$row[kd_pr]\")'>Peserta</button>
                      <button type=\"button\" class=\"btn btn-danger\" onclick='tolak(\"$row[kd_pr]\")'>Tidak</button></td>";
                  }
                  ?>
                </tbody>
              </table>
              <?php
                if (isset($_GET['peserta'])){
                   $setuju = mysqli_query($con,"UPDATE peserta_reguler SET status='1' WHERE kd_pr='$_GET[peserta]'");
                }
              ?>
              <?php
                if (isset($_GET['tolak'])){
                    $setuju = mysqli_query($con,"DELETE FROM peserta_reguler WHERE kd_pr='$_GET[tolak]'");
   ...
   <script>
      ...
      swal({
          ...
          showLoaderOnConfirm: true,
              ...
              $.ajax({
                 type: "POST",
                 url: "http://localhost/sispenal/admin/pendaftar_reguler.php?peserta="+id
              }).done(function( data ) {
                swal("Berhasil!!", "Data Resmi Jadi Peserta", "success");
              setTimeout(function(){
                location.reload();
                }, 2000);
              });
